<h1 align="center">Валидация данных, обучение и интерпретация модели классификации изображений MNIST</h1>

<p align="center">
  Отчёт по лабораторной работе&nbsp;№&nbsp;1 &middot; дисциплина «Валидация систем искусственного интеллекта»<br/>
  Университет ИТМО &middot; Санкт-Петербург, 2026
</p>

<p align="center">
  <b>Выполнила:</b> магистрант гр. M4145 Деревицкая Полина Кирилловна &nbsp;·&nbsp; <b>Преподаватель:</b> доцент ФБИТ, к.т.н. Попов И.&nbsp;Ю.
</p>
...
Три итоговые конфигурации обучены также в JAX (Flax + Optax) в сопоставимых
условиях: та же архитектура, оптимизатор AdamW, число эпох и критерий остановки,
единая тестовая выборка и **общий порядок мини-пакетов** (seed-перестановка NumPy).

**Особенности реализации.** PyTorch — императивный стиль: модель хранит параметры,
цикл обучения явно вызывает `loss.backward()` и `optimizer.step()`. JAX/Flax —
функциональный: параметры вынесены наружу и передаются явно, шаг обучения —
чистая jit-компилируемая функция с `jax.value_and_grad`, dropout получает явный
PRNG-ключ (в PyTorch случайность dropout скрыта в глобальном генераторе). Работа с
данными и оптимизаторами в обоих фреймворках удобна; автодифференцирование в JAX
через `jax.grad` особенно лаконично для градиентных методов.

**Метрики и стабильность** (таблица 8, рисунки 20–21). Результаты фреймворков
близки: разница по test macro-F1 не превышает ~0,3 п.п. При трёх запусках с разными
seed стандартное отклонение macro-F1 составляет ~0,001 у обоих фреймворков — обучение
устойчиво.

Таблица 8 – Сравнение PyTorch и JAX (итоговые конфигурации)

| Набор | Фреймворк | Test macro-F1 | Macro-F1 (среднее±ст.откл., 3 запуска) | Время обуч., с |
|---|---|---:|---|---:|
| original | PyTorch | 0,9788 | 0,977 ± 0,001 | 66 |
| original | JAX | 0,9821 | 0,981 ± 0,001 | 87 |
| balanced | PyTorch | 0,9745 | 0,973 ± 0,001 | 34 |
| balanced | JAX | 0,9754 | 0,974 ± 0,001 | 28 |
| cleaned | PyTorch | 0,9707 | 0,970 ± 0,001 | 30 |
| cleaned | JAX | 0,9692 | 0,971 ± 0,001 | 24 |

![Рисунок 20 – Сравнение PyTorch и JAX по macro-F1](figures/part4_framework_bars.png)

![Рисунок 21 – Стабильность результатов по трём запускам](figures/part4_stability.png)

**Характерные ошибки.** Согласованность предсказаний фреймворков — 98,4–98,6 %;
множества ошибок сильно пересекаются (коэффициент Жаккара 0,53–0,62), то есть модели
ошибаются в основном на одних и тех же трудных объектах. Расхождения объясняются
различиями инициализации весов, реализацией dropout, численной точностью и
особенностями оптимизаторов — при одинаковом порядке батчей и прочих условиях они
малы (`reports/tables/part4_error_overlap.csv`).

**Дополнительный метод — градиентный анализ чувствительности.** Реализованы Saliency
($|\partial\,\text{logit}/\partial x|$), Input×Gradient и Integrated Gradients — и в
PyTorch (autograd), и в JAX (`jax.grad`). Карты приведены на рисунке 22. Карты
saliency, полученные в двух фреймворках, сильно коррелируют (Пирсон 0,68–0,81), что
подтверждает эквивалентность обученных моделей. Корреляция градиентных карт с картами
окклюзии слабо положительная (≈0,07–0,21): методы освещают разные аспекты — окклюзия
измеряет падение вероятности при закрытии области, а градиент — локальную
чувствительность логита; градиентные методы на порядок быстрее окклюзии (один—десятки
проходов против сотен). Значения — в `reports/tables/part4_map_correlation.csv`.

![Рисунок 22 – Градиентные карты (Saliency, Input×Gradient, Integrated Gradients), PyTorch и JAX](figures/part4_gradients_class3.png)

# Итоговая сводка и выводы

- **Исходный набор MNIST (вариант V3), векторное представление 784.** Изображения
  28×28 развёрнуты в векторы длины 784; представление сохраняет яркость, но теряет
  для полносвязной сети пространственную структуру и инвариантность к сдвигу.
- **Выявленные проблемы данных.** Сильный дисбаланс (класс 3 — 9 997, класс 9 —
  2 083; коэффициент дисбаланса 4,80), 3 866 полных дубликатов строк (все в классе 3),
  ~2,35 % аномальных объектов (выбросы по интенсивности/заполненности, далёкие от
  центроида, kNN- и Isolation-Forest-выбросы в пространстве PCA); конфликтов меток и
  утечки теста в обучение нет.
- **Балансировка и фильтрация.** Undersampling до размера наименьшего класса;
  очищенный набор дополнительно избавлен от дубликатов и аномалий (сохранены отдельно).
- **Архитектура.** Двухслойный перцептрон 784→256→128→10 (235 146 параметров),
  активации relu/tanh/gelu, dropout, softmax вне модели.
- **Фреймворк и обучение.** PyTorch и JAX; AdamW (lr 1·10⁻³, wd 1·10⁻⁴), пакет 128,
  до 30 эпох, ранняя остановка по val macro-F1; исследованы dropout p ∈ {0; 0,2; 0,5}.
- **Итоговые метрики.** Лучшая модель (original, tanh, p=0) — test accuracy 0,979,
  macro-F1 0,979; сбалансированная и очищенная — 0,975 и 0,971.
- **Ключевые ошибки** — классические смешения похожих цифр (9→4, 9→7, 3→5, 7→1).
- **Аномалии.** В большинстве по-прежнему уверенно и верно классифицируются
  (в среднем 2,74 из 3 моделей согласны с исходной меткой) — поэтому очистка почти не
  меняет метрики.
- **Интерпретация.** Окклюзия и карты весов показывают, что модель опирается на
  осмысленные штрихи цифр; dropout снижает «зашумлённость» карт весов. Градиентные
  методы согласуются между фреймворками и дополняют окклюзию.
- **Влияние качества данных.** На «чистом» MNIST с простой моделью объём данных
  важнее балансировки: модель на большем (несбалансированном) наборе точнее. Dropout
  полезен на меньших наборах. Балансировка и фильтрация повышают честность оценки и
  устойчивость, но при таком сокращении объёма немного снижают итоговые метрики.
- **PyTorch vs JAX.** Результаты сопоставимы (разница ≤0,3 п.п.), устойчивы по seed,
  ошибки и карты чувствительности сильно коррелируют.
- **Ограничения и улучшения.** Полносвязная модель не использует пространственную
  структуру и чувствительна к сдвигам; улучшения — свёрточная сеть, аугментации,
  взвешенная функция потерь вместо undersampling, калибровка уверенности.
